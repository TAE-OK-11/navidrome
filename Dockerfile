...
FROM --platform=$BUILDPLATFORM public.ecr.aws/docker/library/golang:1.26.4-alpine AS build-alpine
COPY --from=xx / /
...
FROM --platform=$BUILDPLATFORM public.ecr.aws/docker/library/golang:1.26.4-trixie AS base
