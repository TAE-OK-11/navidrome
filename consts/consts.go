...
const (
	...
	DefaultListenBrainzLabsBaseURL     = "https://labs.api.listenbrainz.org/"
...
var (
	...
	DefaultTranscodings       = []struct {
	...
	}{
		{
			...
			TargetFormat:   "mp3",
			DefaultBitRate: 192,
			Command:        "ffmpeg -nostdin -hide_banner -ss %t -i %s -map 0:a:0 -vn -sn -dn -map_metadata 0 -map_metadata 0:s:a:0 -b:a %bk -v 0 -f mp3 -",
		},
		{
			Name:           "opus audio",
			TargetFormat:   "opus",
			DefaultBitRate: 128,
			Command:        "ffmpeg -nostdin -hide_banner -ss %t -i %s -map 0:a:0 -vn -sn -dn -map_metadata 0 -map_metadata 0:s:a:0 -b:a %bk -v 0 -c:a libopus -f opus -",
		},
		{
			Name:           "aac audio",
			TargetFormat:   "aac",
			DefaultBitRate: 256,
			Command:        "ffmpeg -nostdin -hide_banner -ss %t -i %s -map 0:a:0 -vn -sn -dn -map_metadata 0 -map_metadata 0:s:a:0 -b:a %bk -v 0 -c:a aac -f adts -",
		},
		{
			Name:           "flac audio",
			TargetFormat:   "flac",
			DefaultBitRate: 0,
			Command:        "ffmpeg -nostdin -hide_banner -ss %t -i %s -map 0:a:0 -vn -sn -dn -map_metadata 0 -map_metadata 0:s:a:0 -v 0 -c:a flac -f flac -",
		},
	}
)
