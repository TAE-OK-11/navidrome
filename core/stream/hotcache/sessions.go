...
//nolint:gocyclo // Session observation is one lock-scoped state transition; splitting it risks inconsistent counters.
...
func (t *sessionTracker) reachThreshold(ctx context.Context, session *playSession) {
	...
	if err := t.owner.queuePromotion(session.identity, session.playedDuration, session.playedPercent, "play-threshold", session.id); err != nil {
		log.Warn(ctx, "Could not queue hot cache promotion", "mediaID", session.identity.mediaID,
