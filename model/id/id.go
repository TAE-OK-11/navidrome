...
// NewRandom returns a 22-char zero-padded base62 encoding of 128 random bits.
// Existing rows keep their previous ids; only newly generated ids use this form.
...
func NewHash(data ...string) string {
	...
	for _, d := range data {
		hash.Write([]byte(d))
		hash.Write([]byte{0xe2, 0x80, 0x8b}) // U+200B, same separator as before
	}
