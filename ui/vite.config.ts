...
const frontendPort = parseInt(process.env.PORT ?? '', 10) || 4533
  ...
  resolve: {
    alias: [
      ...
      },
      {
        ...
        find: /^@mui\/material$/,
        replacement: fileURLToPath(
          new URL('./src/compat/muiMaterial.tsx', import.meta.url),
        ),
      },
      {
        find: /^@mui\/icons-material$/,
        replacement: fileURLToPath(
          new URL('./src/compat/muiIcons.ts', import.meta.url),
        ),
      },
    ],
  },
...
// PWA manifest
function manifest(): Partial<ManifestOptions> {
  return {
    name: 'Navidrome',
